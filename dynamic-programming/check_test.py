        ...
        for s in range(env.nS):
            ...
            for a, action_prob in enumerate(policy[s]):
                for transition in env.P[s][a]:
                    # 适配新版 gym 的转移元组格式
                    # 检查转移元组的长度来确定格式
                    if len(transition) == 4:  # 旧格式: (prob, next_state, reward, done)
                        prob, next_state, reward, done = transition
                    else:  # 新格式: (prob, next_state, reward, terminated, truncated)
                        prob, next_state, reward, terminated, _ = transition
